...
class ClaudeClient:
    ...
    async def send_message(self, text: str, image_paths: list[str] | None = None, file_paths: list[str] | None = None, expect_image: bool = False, image_model: str = "", chat_model: str = "") -> ChatResponse:
        """
        ...
        Args:
            ...
            chat_model: Ignored. Claude exposes a single chat model id here; the
                parameter keeps the client interface uniform (Gemini/Grok use it
                to drive their in-UI model pickers).
        ...
        # 3. Paste the message
        await verify_temporary_chat(self._page)
        await enter_prompt(self._page, text, [input_selector], name="Claude")
        ...
        # 4. Send the message
        await submit_prompt(
            self._page, text, ClaudeSelectors.CHAT_INPUT,
            ClaudeSelectors.SEND_BUTTON, name="Claude",
            stop_selectors=ClaudeSelectors.STOP_BUTTON,
        )
        ...
        if not completed:
            ...
            if not latest_signature or latest_signature == pre_turn_signature:
                raise RuntimeError("Claude: no new assistant response before timeout")
