...
from src.core.browser.human import human_click, random_delay
from src.providers.composer import enter_prompt, submit_prompt
from src.providers.chatgpt.detector import (
...
from src.providers.chatgpt.image_handler import extract_images_from_response
from src.providers.chatgpt.models import ChatResponse
from src.providers.chatgpt import chat_models as M
...
class ChatGPTClient:
    ...
    async def send_message(
        ...
        Args:
            ...
            expect_image: True when the caller asked for image generation. Lets
                the extractor poll for the image instead of checking once —
                only worth the added latency when an image is actually coming.
            chat_model: ChatGPT picker entry (current selection if empty).
        ...
        # Switching models may reset the thread. Select before taking a baseline
        # or uploading attachments, just as Qwen and DeepSeek do.
        await self._dismiss_overlays()
        wanted_model = M.resolve_chat_model_id(chat_model)
        self.last_chat_model = ""
        labels = M.label_candidates(wanted_model)
        if labels and await pick_from_dropdown(
            self._page, open_selectors=Selectors.MODEL_MENU_BUTTON,
            item_selectors=Selectors.MODEL_MENU_ITEM, labels=labels,
            submenu_labels=["Legacy models", "More models", "Устаревшие модели", "Другие модели"],
            exact=True, log=log, what=f"ChatGPT model {wanted_model}",
        ):
            self.last_chat_model = wanted_model
        ...
        await enter_prompt(
            self._page, text, [input_selector], name="ChatGPT", prefer_fill=True
        )

        # 4. Keep the guard against frontends that submit automatically after
        ...
        if auto_submitted:
            ...
            await submit_prompt(
                self._page, text, Selectors.CHAT_INPUT, Selectors.SEND_BUTTON,
                name="ChatGPT", user_selectors=["[data-message-author-role='user']"],
                stop_selectors=Selectors.STOP_BUTTON,
            )
        ...
        completed = await wait_for_response_complete(
            ...
            timeout_ms=Config.LONG_MODE_TIMEOUT if M.is_long(wanted_model) else Config.RESPONSE_TIMEOUT,
        )

        if not completed:
            latest_signature = await get_latest_assistant_turn_signature(self._page)
            if not latest_signature or latest_signature == pre_turn_signature:
                raise RuntimeError("ChatGPT: no new assistant response before timeout")
        ...
        images = await extract_images_from_response(self._page, expect_image=expect_image)
        ...
        # Best-effort usage-limit detection on the reply text. A throttled reply
        # produces no useful answer, so we flag it for the account layer to put
        # this account on cooldown. Only meaningful when NO image came back
        # (an image reply is a success, not a limit).
        limit = LimitInfo(hit=False)
        if not has_images:
            limit = detect_limit(response_text)
            if limit.hit:
                log.warning(
                    f"Usage-limit banner detected (kind={limit.kind}, "
                    f"reset={int(limit.reset_seconds)}s): {response_text[:120]}"
                )
    ...
    async def navigate_to_thread(self, thread_id: str) -> None:
        ...
        Waits for the conversation's turns to actually render before returning.
        This matters: the very next thing send_message() does is count the
        existing assistant turns to know how many to wait for. If we return
        while the DOM is still empty, that count is 0, send_message thinks the
        reply is already present, and it extracts a STALE turn — the follow-up
        would get the previous answer instead of the new one.
        """
        url = f"{Config.CHATGPT_URL}/c/{thread_id}"
        log.info(f"Navigating to thread: {thread_id}")
        await self._page.goto(url, wait_until="domcontentloaded")

        # Wait for the existing turns to appear, then for their count to settle
        # (a resumed thread streams its history into the DOM progressively).
        try:
            await self._page.wait_for_selector(
                'section[data-testid^="conversation-turn-"]', timeout=15000
            )
        except Exception:
            log.warning(f"Thread {thread_id}: no turns rendered within 15s")

        prev = -1
        for _ in range(20):  # up to ~4s of stabilization
            await asyncio.sleep(0.2)
            count = await count_assistant_messages(self._page)
            if count == prev and count > 0:
                break
            prev = count
        log.info(f"Thread {thread_id} loaded ({prev} assistant turn(s))")
