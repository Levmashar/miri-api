{
  description = "miri-api browser-automation gateway";
    ...
    flake-utils.lib.eachSystem [ "x86_64-linux" ] (
          ...
          pkgs.writeShellApplication {
            ...
            text = ''
              ...
              if [ "$runtime_root" = "${src}" ] && [ ! -f "$runtime_root/.env" ]; then
                export BROWSER_DATA_DIR="''${BROWSER_DATA_DIR:-$state_home/chatgpt/browser_data}"
                export LOG_DIR="''${LOG_DIR:-$state_home/chatgpt/logs}"
                export IMAGES_DIR="''${IMAGES_DIR:-$state_home/chatgpt/downloads/images}"
              fi
      ...
      in
      {
        ...
        apps = {
          default = {
            type = "app";
            program = "${proxyScript}/bin/chatgpt-proxy";
            meta.description = "Run miri-api FastAPI proxy";
          };
          proxy = {
            type = "app";
            program = "${proxyScript}/bin/chatgpt-proxy";
          ...
          login = {
            type = "app";
            program = "${loginScript}/bin/chatgpt-login";
            meta.description = "Run one-time ChatGPT login flow";
          };
          tui = {
            type = "app";
            program = "${tuiScript}/bin/chatgpt-tui";
            meta.description = "Run miri-api terminal UI";
          };
        };

        checks.imports =
          pkgs.runCommand "chatgpt-import-check"
            {
              nativeBuildInputs = [
                pythonEnv
                pkgs.nodejs
