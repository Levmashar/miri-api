...
This guide covers every way to run miri-api: Docker, local development, and Nix.
...
```bash
# 1. Clone the repo
git clone https://github.com/GautamVhavle/miri-api.git
cd miri-api
...
- **Code is baked into the image.** After editing source files, rebuild:
  ...
  ```
  `docker restart chatgpt` does NOT pick up code changes.

- **Browser session persists** via the `chatgpt_browser_data` Docker volume. You only need to log in once.

- **Logs** are bind-mounted to `./docker-logs/` on the host.

- **noVNC** at `http://localhost:6080` lets you see and interact with the browser (useful for debugging, CAPTCHAs, or re-login). Default VNC password: `chatgpt`.
...
miri-api uses your existing browser session. You sign in **once** and the browser profile is persisted.
...
7. Close the noVNC tab — your session is saved in the `chatgpt_browser_data` Docker volume and survives container restarts.
...
Default: `chatgpt`. Change it via `VNC_PASSWORD` in `.env` or `docker-compose.yml`.
...
| Volume | Purpose |
|---|---|
| `chatgpt_browser_data:/app/browser_data` | Persistent browser session (cookies, login) |
...
```bash
docker inspect --format='{{.State.Health.Status}}' chatgpt
```
...
```ini
# ~/.config/systemd/user/chatgpt.service
[Unit]
Description=miri-api
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
WorkingDirectory=%h/Projects/miri-api
...
systemctl --user enable --now chatgpt
journalctl --user -u chatgpt -f
```
...
```bash
# Check logs
docker logs chatgpt --tail 50      # Docker
...
```bash
docker exec chatgpt cat /etc/hosts
docker exec chatgpt curl -s https://chatgpt.com
```
...
# NOT: docker restart chatgpt   # this uses the old image
```

### Services not running

```bash
docker exec chatgpt supervisorctl status
```

All 4 services (xvfb, vnc, novnc, chatgpt) should show `RUNNING`.
